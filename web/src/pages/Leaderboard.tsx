...
export default function Leaderboard() {
  ...
  const fetchLeaderboard = useCallback(async () => {
    try {
      const data = await api.get<LeaderboardEntry[]>(`/parties/${partyId}/leaderboard`);
      setEntries(data);
      setError("");
    } catch (err) {
      ...
      <div className="page animate-fade-in-up">
        ...
        <div className="page-content">
          <div className="empty-state">
            ...
            <p className="empty-state-text">Scores will appear here once predictions are locked in and results are revealed.</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="page animate-fade-in-up">
      ...
      <div className="page-content">
        <div className="stagger-children" style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
          {entries.map((entry) => (
            <LeaderboardRow key={entry.userId} entry={entry} isCurrentUser={entry.userId === userId} />
          ))}
        </div>
      </div>
    </div>
